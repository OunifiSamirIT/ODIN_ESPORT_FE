              ...
              <div className="card-body flex flex-col  d-flex p-0">
                ...
                >
                  <div className="card-body p-0 d-flex">
                    <Link to={`/profile/${originalArticle?.userspartage?.id}`}>
                      <figure className="avatar me-3">
                        <img
                          srcSet={originalArticle?.userspartage?.image ? originalArticle?.userspartage?.image : placeholder}
                    ...
                    <h4 className="fw-700 text-grey-900 font-xssss mt-1">
                      ...
                      <span className="d-block font-xssss fw-500 mt-1 lh-3 text-grey-500">
                        {originalArticle?.userspartage?.profil == 'other' ? originalArticle?.userspartage?.other?.profession : ''}
                        {originalArticle?.userspartage?.profil == 'player' ? ' Joueur' : ''}
                        {originalArticle?.userspartage?.profil == 'coach' ? ' Entraîneur' : ''}
                        {originalArticle?.userspartage?.profil == 'agent' && originalArticle?.userspartage?.agent?.typeresponsable == 'players' ? 'Manager de Joueur' : ''}
                        {originalArticle?.userspartage?.profil == 'agent' && originalArticle?.userspartage?.agent?.typeresponsable == 'club' ? 'Manager de Club' : ''}
                        {originalArticle?.userspartage?.profil == 'scout' ? 'Scout' : ''}
                      </span>
                      <span className="d-block font-xssss fw-500 mt-1 lh-3 text-grey-500">
                        {moment(originalArticle?.createdAt).format('DD MMMM YYYY')} {'  -  '}
                        {
                          moment(originalArticle?.createdAt).isAfter(moment().subtract(1, 'hour')) ?
                  ...
                  <div className="p-0 mt-2">
                    <p className="rounded-md break-inside-avoid-page text-wrap text-base w-full mb-2 text-dark">
                      {!showFullText && originalArticle?.description?.length > 295 ? originalArticle?.description?.substring(0, 295) + "..." : originalArticle.description}
                  ...
                  >
                    <div className="card-body p-0 d-flex">
                      <Link to={`/profile/${articlesPartage?.user?.user?.id}`}>
                        <figure className="avatar me-3">
                          <img
                            srcSet={articlesPartage?.user?.user?.image ? articlesPartage?.user?.user?.image : placeholder}
                      ...
                      <h4 className="fw-700 text-grey-900 font-xssss mt-1">
                        <Link to={`/profile/${articlesPartage?.user?.user?.id}`}>
                          ...
                          {articlesPartage?.user?.user?.prenom} </Link>
                        <span className="d-block font-xssss fw-500 mt-1 lh-3 text-grey-500">
                          {articlesPartage?.user?.user?.profil == 'other' ? articlesPartage?.user?.other?.profession : ''}
                          {articlesPartage?.user?.user?.profil == 'player' ? ' Joueur' : ''}
                          {articlesPartage?.user?.user?.profil == 'coach' ? ' Entraîneur' : ''}
                          {articlesPartage?.user?.user?.profil == 'agent' && articlesPartage?.user?.agent?.typeresponsable == 'players' ? 'Manager de Joueur' : ''}
                          {articlesPartage?.user?.user?.profil == 'agent' && articlesPartage?.user?.agent?.typeresponsable == 'club' ? 'Manager de CLub' : ''}
                          {articlesPartage?.user?.user?.profil == 'scout' ? 'Scout' : ''}
                        </span>

                        <span className="d-block font-xssss fw-500 mt-1 lh-3 text-grey-500">
                          {moment(articlesPartage?.createdAt).format('DD MMMM YYYY')} {'  -  '}
                          {
                            moment(articlesPartage?.createdAt).isAfter(moment().subtract(1, 'hour')) ?
                    ...
                    <div class=" p-0  mt-2">
                      <p className="rounded-md break-inside-avoid-page text-wrap text-base w-full mb-2 text-dark">
                        {!showFullText && articlesPartage?.description?.length > 295 ? articlesPartage?.description?.substring(0, 295) + "..." : articlesPartage?.description}
