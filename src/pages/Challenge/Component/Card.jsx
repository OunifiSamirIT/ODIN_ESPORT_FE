...
const Card = ({ item }) => {
    ...
    return (
        <Link to={`/challenges/details/${item.id}`} className="cursor-pointer flex flex-col flex-wrap content-start p-6 mx-auto w-full bg-red-500 font-light bg-white rounded-[10px] text-zinc-900">
            <div class="relative">
                <video ref={video} onClick={playVideo} class="object-cover w-full aspect-square"
                    src={`${item.video}`}
                    onLoaded={captureThumbnail}>
                    {thumbnailUrl && <img src={thumbnailUrl} alt="Video Thumbnail" />}
                </video>
                {!isPlaying && <svg onClick={playVideo} className="top-[35%] left-[35%] absolute flex  cursor-pointer" width="79" height="78" viewBox="0 0 79 78" fill="none" xmlns="http://www.w3.org/2000/svg">
