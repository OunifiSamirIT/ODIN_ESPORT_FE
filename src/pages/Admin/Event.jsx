...
const Event = () => {
  ...
  useEffect(() => {
    const fetchEvents = async () => {
      try {
        const response = await fetch(`${Config.LOCAL_URL}/api/event`);
        const result = await response.json();
                  ...
                  <div key={index} className="col-lg-4 col-md-6 pe-2 ps-2 mt-2">
                    <div className="card p-3 h-100 bg-white w-100 hover-card border-0 shadow-xss  rounded-xxl border-0 mb-3 overflow-hidden ">
                      <div className="card-image object-cover cover h150">
                        <img
                          src={value.ImageEvents[0].image_url}
                          alt="event"
                          className="rounded-3 card-body position-relative "
                        />
                      </div>
                      <div className="card-body d-flex ps-0 pe-0 pb-0">
                        <div className="bg-greylight me-3 p-3 border-light-md rounded-xxl theme-dark-bg d-flex justify-items-center align-items-center text-center">
                      ...
                      <div className="card-body p-0">
                        ...
                        <button
                          onClick={() => {
                            hanleEventApplication(value.id, storedUserData.id);
                          }}
