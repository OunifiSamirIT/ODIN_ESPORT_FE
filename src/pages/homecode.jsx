...
function Home() {
  ...
  const handleLikeClick = async (articleId, emoji) => {
    try {
      ...
      if (response.ok) {
        ...
        const allLikesResponse = await fetch("https://odine-sport.com/api/likes/article/allLikes");
        const allLikesData = await allLikesResponse.json();
  
        // Update the state based on the received likes count
        setArticles((prevArticles) =>
          prevArticles.map((article) => {
            const updatedLikesCount = allLikesData.find((like) => like.articleId === article.id)?.likesCount || 0;
            return article.id === articleId ? { ...article, likesCount: updatedLikesCount } : article;
          })
        );
      } else {
        toast.error("Error liking/unliking the article. Please try again.", {
        ...
        setArticles((prevArticles) =>
          prevArticles.map((article) => {
            const updatedComments = article.comments.map((c) =>
              c.id === commentId ? { ...c, likesCount: likesCountData.count } : c
            );
  ...
  const fetchArticles = async () => {
    try {
      ...
      const comtResponse = await Promise.all(
        comt.map((articleId) =>
          fetch(`https://odine-sport.com/api/commentaires/article/${articleId}`).then((response) =>
            response.json()
          )
        )
      );
  ...
  const fetchCommentsByArticleId = async (articleId) => {
    try {
      const response = await fetch(`https://odine-sport.com/api/commentaires/article/${articleId}`);
