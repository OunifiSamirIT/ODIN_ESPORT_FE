...
const Social = ({ userInfo }) => {
    ...
    const schema = yup
        .object({
            facebook: yup.string().required('Ce champ est obligatoire').max(50, ({ max }) => `Maximum de (${max} characters autorisé)`),
            instagram: yup.string().max(255, ({ max }) => `Maximum de (${max} characters autorisé)`),
    ...
    const handleSocialUpdate = () => {
        ...
        const response = fetch(`${Config.LOCAL_URL}/api/user/${LocalStorageID.id}`, {
            method: "PUT",
            body: formData,
        }).then((r) => {
            console.log(r)
            ...
            <div className="flex flex-col px-8 py-9 text-lg bg-white rounded-xl text-zinc-900 max-md:px-5 max-md:max-w-full">
                <div className="grid gap-4 grid-cols-1 md:grid-cols-2">
                    <div className="col-span-1 flex flex-col basis-0 text-zinc-900">
                        ...
                        <input name='facebook' {...register('facebook')} value={facebook} onChange={(e) => setFacebook(e.target.value)} placeholder="joeyleblanc" className="justify-center items-start py-3.5 pr-16 pl-4 mt-2 text-base border border-solid border-[color:var(--black-100-e-5-e-5-e-5,#E5E5E5)] rounded-[30px] max-md:pr-5" />
                    </div>
                    <div className="col-span-1 flex flex-col basis-0 text-zinc-900">
                        <div className="flex gap-4 justify-between px-4 text-lg">
                            <img
                        ...
                        <input name='instagram' {...register('instagram')} value={instagram} onChange={(e) => setInstgram(e.target.value)} placeholder="joeyleblanc" className="justify-center items-start py-3.5 pr-16 pl-4 mt-2 text-base border border-solid border-[color:var(--black-100-e-5-e-5-e-5,#E5E5E5)] rounded-[30px] max-md:pr-5" />
                    </div>
                    <div className="col-span-1 flex flex-col basis-0 text-zinc-900">
                        <div className="flex gap-4 justify-between px-4 text-lg">
                        ...
                        <input name='twitter' {...register('twitter')}  value={twitter} onChange={(e) => setTwitter(e.target.value)} placeholder="joeyleblanc" className="justify-center items-start py-3.5 pr-16 pl-4 mt-2 text-base border border-solid border-[color:var(--black-100-e-5-e-5-e-5,#E5E5E5)] rounded-[30px] max-md:pr-5" />
                    </div>
                    <div className="col-span-1 flex flex-col basis-0 text-zinc-900">
                        <div className="flex gap-4 justify-between px-4 text-lg">
                            <svg width="25" height="25" viewBox="0 0 25 25" fill="none" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <input name='tiktok' {...register('tiktok')} value={tiktok} onChange={(e) => setTiktok(e.target.value)} placeholder="@joeyleblanc" className="justify-center items-start py-3.5 pr-16 pl-4 mt-2 text-base border border-solid border-[color:var(--black-100-e-5-e-5-e-5,#E5E5E5)] rounded-[30px] max-md:pr-5" />
                    </div>
                    <div className="col-span-1 flex flex-col basis-0 text-zinc-900">
                        <div className="flex gap-4 justify-between px-4 text-lg">
                            <svg width="25" height="25" viewBox="0 0 25 25" fill="none" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <input name='linkedin' {...register('linkedin')} value={linkedIn} onChange={(e) => setLinkedIn(e.target.value)} placeholder="joeyleblanc" className="justify-center items-start py-3.5 pr-16 pl-4 mt-2 text-base border border-solid border-[color:var(--black-100-e-5-e-5-e-5,#E5E5E5)] rounded-[30px] max-md:pr-5" />
                    </div>
                </div>
                <div className="flex gap-2 justify-between py-2 mt-6 mr-4 w-full text-base font-medium whitespace-nowrap">
                    ...
                    <div className="flex gap-2 justify-between px-8 py-2 text-white bg-blue-600 rounded-[30px] max-md:px-5">
                        <img
                            ...
                            className="w-5 aspect-square"
                        />
                        <button onClick={handleSocialUpdate} className="grow">Confirmer</button>
                    </div>
                </div>
            </div>
        </>
