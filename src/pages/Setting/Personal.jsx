...
const Personal = ({ userInfo }) => {
  ...
  const schema = yup
    .object({
      ...
      discreptionBio: yup
        ...
        .max(
          255,
          ({ max }) => `Vous ne pouvez pas dépasser les  ${max} characters`
        )
        .nullable(),
      prenom: yup
        .string()
      ...
      gender: yup
        .string()
        .oneOf(["male", "female"], "Ce champ est obligatoire"),
  ...
  const onSubmit = async (data) => {
    ...
    formDataToUpdate.append(
      "discreptionBio",
      data.discreptionBio ? data.discreptionBio : ""
    );
    ...
    formDataToUpdate.append(
      "date_naissance",
      new Date(data.date_naissance).getFullYear()
    );
    formDataToUpdate.append("gender", data.gender);
    formDataToUpdate.append(
      "countryresidence",
      data.country?.label?.props?.children[1]
    );
    // formDataToUpdate.append("tel", data.tel);
    formDataToUpdate.append(
      "nationality",
      data.nationality?.label?.props?.children[1]
    );
    formDataToUpdate.append("cityresidence", data.cityresidence);
    formDataToUpdate.append("image", file);
    formDataToUpdate.append(
      "prefix",
      `${data.wats?.label?.props?.children[2]},${data.phoneLength?.label?.props?.children[2]}`
    );
    formDataToUpdate.append(
      "langueparlee",
      data.langueparlee !== undefined
  ...
  const selectedObject = (countryName) => {
    return (
      optionsphone.find(
        (item) => item.label.props?.children[2] == countryName
      ) || ""
    );
  };
  const resetForm = async () => {
    ...
    setValue(
      "discreptionBio",
      userInfo.user.discreptionBio !== null ? userInfo.user.discreptionBio : ""
    );
    ...
    setValue(
      "phoneLength",
      selectedObject(userInfo.user?.optionalattributs.split(",")[0]) || null
    );
    ...
    setValue(
      "phoneLength",
      selectedObject(userInfo.user?.optionalattributs.split(",")[1]) || null
    );
  ...
  useEffect(() => {
    ...
    const defaultValueN = (countryName) => {
      return optionsCountry.find(
        (option) => option.label.props?.children[1] === countryName
      );
    };
    fetch(`${Config.LOCAL_URL}/api/user/${storedUserData.id}`)
      ...
      .then((userData) => {
        ...
        setValue(
          "discreptionBio",
          userData?.user.discreptionBio != null
            ? userData?.user?.discreptionBio
            : ""
        );
        ...
        setValue(
          "wats",
          selectedObject(userData.user?.optionalattributs.split(",")[0]) || null
        );
        const selectedOptions = userData?.user?.langueparlee
          ?.split(",")
          .map((lang) => ({
            value: lang,
            label: lang,
        ...
        setValue(
          "phoneLength",
          selectedObject(userData.user?.optionalattributs.split(",")[1]) || null
        );
      })
  ...
  const handleChangePhoneNumberWS = (e) => {
    const value = e.target.value
      .replace(/\D/g, "")
      .slice(0, whatsappLength?.phoneLength ? whatsappLength.phoneLength : 0);
    setValue("numWSup", value);
    const textError = `Le Numéro doit être avec ${
      whatsappLength?.phoneLength ? whatsappLength?.phoneLength : 5
    } chiffres.`;
  ...
  const handleChangePhoneNumber = (e) => {
    const value = e.target.value
      .replace(/\D/g, "")
      .slice(0, phoneLength?.phoneLength ? phoneLength.phoneLength : 5);
    const textError = `Le Numéro doit être avec ${
      phoneLength?.phoneLength ? phoneLength?.phoneLength : 5
    } chiffres.`;
  ...
  const optionsLangue = Array.from(
    new Set(Langue.map((langue) => langue.name))
  ).map((name) => ({
    value: name,
    label: name,
  }));
  ...
  const handleLanguageChange = (selectedOptions) => {
    ...
    const selectedLanguageLabels = selectedOptions.map(
      (option) => option.value
    );
  };
  return (
    <>
      ...
      <div className="flex flex-col flex-wrap grow gap-y-6 justify-between w-full bg-white rounded-xl">
        <form
          ...
          className="flex flex-col flex-wrap grow gap-y-6 justify-between content-start  w-full bg-white rounded-xl "
        >
              ...
              <label className="rounded-full border-2 aspect-square w-full max-w-[178px] max-md:mt-10">
                {user?.image !== null ? (
                  ...
                  <img
                    loading="lazy"
                    alt="logo"
                    srcSet={imagePreview ? imagePreview : PlaceHolder}
                    className="rounded-full border-2 aspect-square w-full max-w-[178px]"
                  />
                )}
              </label>
          ...
          <div className="flex flex-col w-full">
            ...
            <textarea
              {...register("discreptionBio")}
              name="discreptionBio"
              placeholder="A Propos de Moi"
              className={`border border-gray-800 border-solid ${
                errors.discreptionBio ? "!border-red-500" : "border-neutral-200"
              } justify-center p-4 mt-2 text-base font-light  rounded-[30px] text-zinc-900 max-md:max-w-full `}
            ></textarea>

            {errors.discreptionBio && (
              <span className="invalid-feedback block py-2 px-2">
                {errors?.discreptionBio?.message}
              </span>
            )}
          </div>
            ...
            <div className="lg:flex-1 w-full">
              <div className="flex flex-col whitespace-nowrap text-zinc-900">
                <div className="flex items-center px-4 gap-4  text-lg">
                  ...
                  >
                    <g clip-path="url(#clip0_1458_7391)">
                      <path
                        d="M0 20.499C0 19.1191 0 17.7396 0 16.3604C0 14.3047 1.3625 12.6391 3.37656 12.2401C3.6451 12.1913 3.91771 12.1684 4.19063 12.1719C6.39688 12.1667 8.60278 12.1667 10.8083 12.1719C12.9063 12.1766 14.5625 13.5615 14.9401 15.6245C14.9812 15.8748 15.0002 16.1282 14.9969 16.3818C15.0021 17.7547 14.9969 19.1276 14.9969 20.5005L0 20.499Z"
                        fill="#1D1E21"
                      />
                      <path
                        d="M8.43266 1.00156C8.7936 1.07969 9.16183 1.13073 9.51391 1.23958C11.6832 1.91667 13.1238 4.00677 12.9972 6.27708C12.8702 8.56302 11.1837 10.4922 8.94412 10.9125C6.13631 11.4417 3.45089 9.52292 3.05037 6.7026C2.65454 3.91615 4.63579 1.35989 7.44308 1.03333C7.48726 1.02554 7.53079 1.01439 7.57329 1L8.43266 1.00156Z"
                        fill="#1D1E21"
                      />
                    </g>
                    <defs>
                ...
                <div className="flex flex-col">
                  <input
                    {...register("nom")}
                    className={`form-control  justify-center items-start py-3.5 pr-16 pl-4 mt-2 text-base border-solid  border-[0.5px] border-[color:var(--black-100-e-5-e-5-e-5,#E5E5E5)] rounded-[30px] max-md:pr-5 ${
              ...
              <div className="flex gap-2 mt-2 text-base h-[52px]">
                <Controller
                  ...
                  render={({ field }) => (
                    <Select
                      ...
                      className={`px-2 text-sm text-center flex border-solid border-[0.5px] rounded-[30px] ${
                        errors.wats ? "boreder border-red-500" : ""
                      }`}
                      placeholder="Préfixe"
                      {...field}
                ...
                <div className={"w-full"} style={{}}>
                  <input
                    ...
                    className={`h-[52px] px-3 w-full form-control grow justify-center gap-2 items-start py-3.5 pl-1 border-solid  border-[0.5px] border-neutral-200 rounded-[30px] max-md:pr-2 ${
                      errors.numWSup ? "is-invalid" : ""
                    }`}
                  />
                </div>
              </div>
              ...
              <div className="flex gap-4 items-center px-4 text-lg">
                ...
                >
                  <g clip-path="url(#clip0_595_43798)">
                    <path
                      d="M0.00069809 5.19844C0.00069809 11.5651 8.44236 19.9984 14.8007 19.9984C16.1924 19.9984 17.4924 19.4734 18.4507 18.5151L19.284 17.5568C20.2507 16.5901 20.2507 14.9568 19.2424 13.9484C19.2174 13.9234 17.209 12.3818 17.209 12.3818C16.209 11.4318 14.634 11.4318 13.6424 12.3818L12.4257 13.3568C9.75903 12.2234 7.86736 10.3234 6.6507 7.5651L7.61736 6.34844C8.5757 5.35677 8.5757 3.77344 7.61736 2.78177C7.61736 2.78177 6.0757 0.773438 6.0507 0.748438C5.04237 -0.259896 3.40903 -0.259896 2.4007 0.748438L1.5257 1.50677C0.525698 2.49844 0.00069809 3.79844 0.00069809 5.1901V5.19844Z"
                      fill="#1D1E21"
                    />
                  </g>
                  <defs>
          ...
          <div className="mr-4 max-md:mr-2.5 max-md:max-w-full flex-col md:flex-row flex gap-4 flex-wrap items-center items-base">
            <div className="lg:flex-1 w-full">
              <div className="flex flex-col grow text-base whitespace-nowrap text-zinc-900">
                ...
                <div
                  className={`w-full flex flex-col justify-center py-px mt-2 w-full border-solid  border-[0.5px] border-[color:var(--black-100-e-5-e-5-e-5,#E5E5E5)] rounded-[30px] ${
            ...
            <div className="lg:flex-1 w-full ">
              ...
              <div className="">
                <select
                  ...
                  name="gender"
                  placeholder="Sexe"
                  className={` text-zinc-600 h-[53px] w-full md:w-full form-control justify-center items-start py-3.5 pr-16 pl-4 mt-2 max-w-full text-base whitespace-nowrap border-solid  border-[0.5px] border-[color:var(--black-100-e-5-e-5-e-5,#E5E5E5)] rounded-[30px] w-[379px] ${
          ...
          <div className="mr-4 max-md:mr-2.5 max-md:max-w-full flex-col md:flex-row flex gap-4 flex-wrap items-center items-base">
            <div className="lg:flex-1 w-full">
              ...
              <div
                className={`flex flex-col justify-center py-1.5 mt-2 w-full text-base border-solid  border-[0.5px] border-[color:var(--black-100-e-5-e-5-e-5,#E5E5E5)] rounded-[30px] ${
            ...
            <div className="lg:flex-1 w-full">
              ...
              <div
                className={`flex flex-col justify-center px-px py-1.5 mt-2 w-full text-base border-solid  border-[0.5px] border-[color:var(--black-100-e-5-e-5-e-5,#E5E5E5)] rounded-[30px] ${
          ...
          <div className="mr-4 max-md:mr-2.5 max-md:max-w-full flex-col md:flex-row flex gap-4 flex-wrap items-center items-base">
            <div className="lg:flex-1 w-full">
              <div className="flex gap-4 items-center px-4 text-lg text-zinc-900">
                ...
                >
                  <g clip-path="url(#clip0_608_33866)">
                    <path
                      d="M15 5.5C15 2.7425 12.7575 0.5 10 0.5C7.2425 0.5 5 2.7425 5 5.5C5 7.9725 6.80583 10.0258 9.16667 10.425V20.5H10.8333V10.425C13.1942 10.0267 15 7.97333 15 5.5Z"
                      fill="black"
                    />
                  </g>
                  <defs>
              ...
              <input
                ...
                className={` text-zinc-600   form-control justify-center items-start py-3.5  pl-4 mt-2 max-w-full text-base whitespace-nowrap border-solid  border-[0.5px] border-[color:var(--black-100-e-5-e-5-e-5,#E5E5E5)] rounded-[30px]  max-md:pr-5 ${
                  errors.cityresidence ? "is-invalid" : ""
                }`}
                placeholder="Ville"
              />
