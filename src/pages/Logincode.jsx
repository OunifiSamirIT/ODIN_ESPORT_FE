...
const onSubmit = async (data) => {
  try {
    ...
    const response = await fetch(`${Config.LOCAL_URL}/api/auth/verify-email`, {
      method: "POST",
      headers: {
...
//           "Content-Type": "application/json",
//         },
//         body: JSON.stringify({ verificationToken: data.verificationToken , userId }), // Use data.verificationToken
//       });
...
//       if (response.ok) {
//         console.log("sdsddsdsdsdsdsdsdsdsdsd", JSON.string)
//         navigate("/login");
//       } else {
//         setErrMsg(result.message);
//       }
...
useEffect(() => {
  const handleOtpForm = () => {
    ...
    const handlePaste = (e) => {
      ...
      const digits = text.replace(/[^0-9]/g, '').slice(0, 6).split(''); // Allow only numbers and limit length to 6
  ...
  <div className="flex items-center justify-center gap-5">
    <input
      type="text"
      {...register("input1", { required: true, minLength: 1, maxLength: 2 })}
      placeholder="_ _"
    ...
    <input
      type="text"
      placeholder="_ _"
      {...register("input2", { required: true, minLength: 1, maxLength: 2 })}
    ...
    <input
      type="text"
      placeholder="_ _"
      {...register("input3", { required: true, minLength: 1, maxLength: 2 })}
      className="w-14 h-14 text-center text-2xl font-bold text-slate-900 bg-slate-100 border border-transparent hover:border-slate-500 appearance-none rounded outline-none focus:bg-white focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100"
    />
  </div>
